...
// Безопасная валидация URL – разрешаем только http, https и относительные ссылки
function safeUrl(url) {
    if (!url) return '#';
    // Относительные ссылки (начинаются с /, ./ или #) пропускаем
    ...
    try {
        ...
        // Блокируем javascript:, data:, file:, about: и т.д.
        return '#';
    } catch(e) {
        // Невалидный URL – лучше заблокировать
        return '#';
    }
}
...
// Markdown парсер с безопасными ссылками
function parseMarkdown(text) {
    ...
    // Ссылки [текст](url) – валидируем URL
    html = html.replace(/\[(.*?)\]\((.*?)\)/g, (match, linkText, rawUrl) => {
        const safeHref = safeUrl(rawUrl);
        return `<a href="${safeHref}" target="_blank" rel="noopener noreferrer" style="color:var(--accent-blue);">${linkText}</a>`;
    });
    
    // Упоминания @username (только латиница/цифры для простоты, но не критично)
...
function initDefaultMessages() {
    ...
    for(let ch of CHANNELS) {
        db[ch.id] = [];
        if(ch.id === "general") db[ch.id].push({ id:generateId(), author:"Система", authorAvatar:"СИ", text:"Привет! Используйте **жирный**, *курсив*, `код`, [безопасные ссылки](https://example.com). Загружайте изображения.", timestamp:now-3600000, edited:false });
...
function updateUnreadCountsForNewMessage(channelId, msgAuthor) {
    if(channelId === currentChannelId && msgAuthor !== currentUsername) {
        ...
        if(msgTime > lastView) unreadCounts[channelId] = (unreadCounts[channelId] || 0) + 1;
...
function addMessage(channelId, msgObj) {
    ...
    persistMessages();
    if(msgObj.author !== currentUsername) updateUnreadCountsForNewMessage(channelId, msgObj.author);
...
function renderMessages() {
    ...
    if(!messages.length) { 
        container.innerHTML = `<div style="text-align:center;padding:30px;">🔍 Ничего не найдено</div>`; 
        return; 
    }
    ...
    document.querySelectorAll('.edit-msg').forEach(btn => {
        btn.addEventListener('click', (e) => { 
            ...
            let newText = prompt("Редактировать сообщение:", oldMsg.text); 
    ...
    document.querySelectorAll('.del-msg').forEach(btn => {
        btn.addEventListener('click', (e) => { 
            e.stopPropagation(); 
            if(confirm("Удалить сообщение?")) deleteMessage(currentChannelId, btn.dataset.id); 
        });
    });
}
...
function switchToChannel(channelId) {
    ...
    document.querySelectorAll('.channel-item').forEach(el => { 
        if(el.dataset.channelId === channelId) el.classList.add('active'); 
...
function promptEditUsername() { 
    let newName = prompt("Имя:", currentUsername); 
    if(newName) setUsername(newName); 
}

function loadUsername() { 
    let stored = localStorage.getItem(STORAGE_USERNAME_KEY); 
    currentUsername = stored ? stored : DEFAULT_USERNAME; 
    setUsername(currentUsername); 
}

// ---------- ФУНКЦИИ ВЗАИМОДЕЙСТВИЯ (упоминания, эмодзи, тема) ----------
...
// ---------- ИНИЦИАЛИЗАЦИЯ (все обработчики здесь) ----------
function init() {
    loadMessages();
    loadUsername();
    renderSidebar();
    ...
    document.getElementById("sendMessageBtn").addEventListener("click", () => { 
        ...
        if(val.trim()) sendOwnMessage(val); 
    });
    // Enter
